...
func GetClipboard(clipboard *models.Clipboard) models.Clipboard {
	...
	security.Db.First(&clipboardTemp, "port = ?", clipboard.Port)
...
func DeleteClipboardFromPassword(clipboard *models.Clipboard) bool {
	var password string
	if security.Db.Model(&models.Clipboard{}).Where("port = ?", clipboard.Port).Select("password").Scan(&password).RowsAffected == 0 {
		return false
	}
	if password == clipboard.Password && password != "" {
		security.ActiveRequestLog()
		security.Db.Where("port", clipboard.Port).Delete(&models.Clipboard{})
		return true
	} else {
		return false
	}
...
func UpdateClipboard(clipboard models.Clipboard) bool {
	var clipboardDB models.Clipboard
	if security.Db.Model(&models.Clipboard{}).First(&clipboardDB, "port = ?", clipboard.Port).RowsAffected == 0 {
		return false
	}

	if clipboard.Password == clipboardDB.Password {
		security.ActiveRequestLog()
		security.Db.Model(&clipboardDB).UpdateColumns(models.Clipboard{
